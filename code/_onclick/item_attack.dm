...
/obj/item/proc/attack(mob/living/M, mob/living/user)
	...
	if(!ishuman(M))
		...
		if(isXeno(M))
			...
			power = armor_damage_reduction(GLOB.xeno_melee, power, X.armor_deflection + X.armor_deflection_buff - X.armor_deflection_debuff, weapon_blade * ARMOR_SHARP_PENETRATION, 0, 0, X.armor_integrity)
			var/armor_punch = armor_break_calculation(GLOB.xeno_melee, power, X.armor_deflection + X.armor_deflection_buff - X.armor_deflection_debuff, weapon_blade * ARMOR_SHARP_PENETRATION, 0, 0, X.armor_integrity)
